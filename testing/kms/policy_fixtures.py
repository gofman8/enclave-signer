"""Testing-only identities and manual KMS rollout of the minimal policy examples."""
import copy
import json
...
def fixture_policies(repo, substitutions, bootstrap_pcr, restore_pcr, phase="transition"):
    ...
    key = policy_template(Path(repo) / "deploy/swap-kms-key-policy.json", substitutions)
    bucket = policy_template(Path(repo) / "deploy/swap-seed-bucket-policy.json", substitutions)
    ...
    # supplied by the minimal production usage example. Normal rollout retires
