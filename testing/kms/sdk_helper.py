...
def verify_sdk_provenance(root, prefix):
    ...
    installed = prefix / "share/kms"
    manifest = (root / "build/kms-dependencies.tsv").read_bytes()
...
class SdkHelper:
    def __init__(self, root, artifacts, args, env):
        ...
        self.name = "kms-sdk-e2e-" + uuid.uuid4().hex[:12]
        self.running = False
        self.image_id = None
    ...
    def start(self):
        ...
        command = ["run", "--detach", "--rm", "--name", self.name,
            ...
            "--mount", f"type=bind,source={prefix},target=/opt/kms,readonly",
        ...
        with (self.artifacts / "sdk-helper-build.log").open("w") as log:
            self.run(["exec", self.name, "cmake", "-GNinja", "-S", "/src/testing/kms",
                "-B", build, "-DCMAKE_BUILD_TYPE=Debug", "-DCMAKE_PREFIX_PATH=/opt/kms",
...
command = CONFIG["command"] + ["exec", "-i", CONFIG["name"], "/usr/bin/env", "-i",
    "LD_LIBRARY_PATH=" + CONFIG["build"] + "/mock:/opt/kms/lib"]
for name in ("KMS_E2E_PCR0", "KMS_E2E_PORT"):
    if name in os.environ:
        command.append(name + "=" + os.environ[name])
if os.environ.get("KMS_E2E_CA_PEM"):
    relative = Path(os.environ["KMS_E2E_CA_PEM"]).resolve().relative_to(Path(CONFIG["artifacts"]))
    command.append("KMS_E2E_CA_PEM=/test-artifacts/" + str(relative))
command.append(CONFIG["build"] + "/bin/kms-tool")
